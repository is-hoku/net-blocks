...
void identifier_module::init_module(void) {
  conn_layout.register_member<builder::dyn_var<unsigned long long>>(
      "remote_host_id");
  ...
  conn_layout.register_member<builder::dyn_var<accept_queue_t *>>(
      "accept_queue");

  // A non-send member
  // Network module ignores this while sending and adds headroom for this while
  // receiving
  net_packet.add_member("flow_identifier",
  ...
  net_state
      .register_member<builder::dyn_var<unsigned int[MAX_ACTIVE_CONNECTIONS]>>(
          "active_local_app_ids");
  if (flow_identifier == flow_identifier_t::src_dst_identifier) {
    net_state.register_member<
        builder::dyn_var<unsigned int[MAX_ACTIVE_CONNECTIONS]>>(
        "active_remote_app_ids");
    net_state.register_member<
        builder::dyn_var<unsigned long long[MAX_ACTIVE_CONNECTIONS]>>(
        "active_remote_host_ids");
  }
  net_state.register_member<
      builder::dyn_var<connection_t *[MAX_ACTIVE_CONNECTIONS]>>(
      "active_connections");
  // Routing table related stuff
  net_state.register_member<builder::dyn_var<runtime::rte_t *>>(
      "routing_table");
...
void identifier_module::hook_net_init(void) {
  ...
  runtime::my_local_host_id = runtime::routing_lookup_from_global(
      runtime::my_host_id, get_state("routing_table"),
      get_state("routing_table_len"));
}

void identifier_module::add_connection(
    builder::dyn_var<connection_t *> c, builder::dyn_var<unsigned int> appid,
...
bool identifier_module::match_connection(
    builder::dyn_var<int> &index, builder::dyn_var<unsigned int> dst_app_id,
    builder::dyn_var<unsigned int> src_app_id,
    builder::dyn_var<unsigned long long> src_host_id) {
  if (flow_identifier == flow_identifier_t::src_dst_identifier) {
...
builder::dyn_var<connection_t *> identifier_module::retrieve_connection(
...
identifier_module::retrieve_wildcard_connection(
    builder::dyn_var<unsigned int> appid) {
  // A wildcard connection is defined as the first partial match we can find
  // The only wildcard pattern we allow is dst_app_id is set and the src_app_id
  // and src_host_id are both wildcard
  builder::dyn_var<connection_t *> c = 0;
  builder::dyn_var<int> total = get_state("num_conn");
  for (builder::dyn_var<int> i = 0; i < total; i = i + 1) {
    if (get_state("active_local_app_ids")[i] == appid &&
        get_state("active_remote_app_ids")[i] == WILDCARD_APP_ID &&
        get_state("active_remote_host_ids")[i] ==
            runtime::wildcard_host_identifier) {
      c = get_state("active_connections")[i];
      break;
    }
  }
...
void identifier_module::delete_connection(
    builder::dyn_var<unsigned int> appid,
    builder::dyn_var<unsigned int> src_app_id,
    builder::dyn_var<unsigned long long> src_host_id) {
  ...
  for (builder::dyn_var<int> i = 0; i < total; i = i + 1) {
    if (match_connection(i, appid, src_app_id, src_host_id)) {
      ...
      get_state("active_local_app_ids")[i] =
          get_state("active_local_app_ids")[total];
      get_state("active_connections")[i] =
          get_state("active_connections")[total];
      if (flow_identifier == flow_identifier_t::src_dst_identifier) {
        get_state("active_remote_app_ids")[i] =
            get_state("active_remote_app_ids")[total];
        get_state("active_remote_host_ids")[total] =
            get_state("active_remote_host_ids")[i];
      }
      break;
    }
...
module::hook_status identifier_module::hook_establish(
  ...
  builder::dyn_var<unsigned long long> host_local_id =
      runtime::routing_lookup_from_global(h, get_state("routing_table"),
                                          get_state("routing_table_len"));
...
module::hook_status
identifier_module::hook_destablish(builder::dyn_var<connection_t *> c) {
  delete_connection(conn_layout.get(c, "local_app_id"),
...
module::hook_status
identifier_module::hook_send(builder::dyn_var<connection_t *> c, packet_t p,
...
module::hook_status identifier_module::hook_ingress(packet_t p) {
  ...
  builder::dyn_var<connection_t *> c =
      retrieve_connection(dst_app_id, src_app_id, src_host_id);
