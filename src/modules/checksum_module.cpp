...
void checksum_module::init_module(void) {
  ...
  net_packet.add_member("checksum",
                        new generic_integer_member<unsigned short>(0), 4);
  framework::instance.register_module(this);
}

static builder::dyn_var<unsigned short>
compute_checksum(packet_t _p, checksum_module::checksum_type_t ct) {
...
module::hook_status
checksum_module::hook_send(builder::dyn_var<connection_t *> c, packet_t p,
                           builder::dyn_var<char *> buff,
                           builder::dyn_var<unsigned int> len,
                           builder::dyn_var<int *> ret_len) {

  builder::dyn_var<unsigned short> checksum =
      compute_checksum(p, checksum_type);
...
module::hook_status checksum_module::hook_ingress(packet_t p) {
  // Save the checksum
  builder::dyn_var<unsigned short> checksum_saved =
      net_packet["checksum"]->get_integer(p);
...
module::hook_status
checksum_module::hook_send_ctx(builder::dyn_var<runtime::context_t *> ctx,
...
module::hook_status
checksum_module::hook_ingress_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                  packet_t p) {
  // Save the checksum
  builder::dyn_var<unsigned short> checksum_saved =
      net_packet["checksum"]->get_integer(p);
