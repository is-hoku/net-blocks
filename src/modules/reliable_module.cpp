...
void reliable_module::init_module(void) {
  ...
  conn_layout.register_member<builder::dyn_var<void *[REDELIVERY_BUFFER_SIZE]>>(
      "redelivery_buffer");
  conn_layout.register_member<builder::dyn_var<unsigned int>>(
      "first_unacked_seq");
  net_packet.add_member(
      "ack_sequence_number",
      new generic_integer_member<unsigned int>((int)member_flags::aligned), 4);
  net_packet.add_member("redelivery_timer",
                        new generic_integer_member<unsigned long long>(
                            (int)member_flags::aligned),
                        0);
...
module::hook_status
reliable_module::hook_establish(builder::dyn_var<connection_t *> c,
...
static builder::dyn_var<void(timer_t *, void *, unsigned long long)>
    redelivery_timer_callback =
        builder::as_global("nb__reliable_redelivery_timer_cb");

static void redelivery_cb(builder::dyn_var<runtime::timer_t *> t,
                          builder::dyn_var<void *> param,
                          builder::dyn_var<unsigned long long> to) {
  packet_t p = param;
  builder::dyn_var<int> size = net_packet["computed_total_len"]->get_integer(p);

  // Get context from timer and use context-aware functions
  builder::dyn_var<runtime::context_t *> ctx = runtime::get_timer_context(t);
  runtime::send_packet_ctx(ctx, p + get_headroom(), size);
  runtime::insert_timer_ctx(ctx, t, to + REDELIVERY_TIMEOUT_MS,
                            redelivery_timer_callback, p);
}

void reliable_module::gen_timer_callback(std::ostream &oss) {
  auto ast = builder::builder_context().extract_function_ast(
      redelivery_cb, "nb__reliable_redelivery_timer_cb");
  block::eliminate_redundant_vars(ast);
  block::c_code_generator::generate_code(ast, oss);
}

module::hook_status
reliable_module::hook_send(builder::dyn_var<connection_t *> c, packet_t p,
  ...
  builder::dyn_var<unsigned int> index =
      net_packet["sequence_number"]->get_integer(p) % REDELIVERY_BUFFER_SIZE;
...
module::hook_status reliable_module::hook_ingress(packet_t p) {
  builder::dyn_var<unsigned int> ack_seq =
      net_packet["ack_sequence_number"]->get_integer(p);
  builder::dyn_var<connection_t *> c =
      runtime::to_void_ptr(net_packet["flow_identifier"]->get_integer(p));
  if (ack_seq != 0) {
    ...
    if (p_rem != 0) {
      builder::dyn_var<runtime::timer_t *> t = runtime::to_void_ptr(
          net_packet["redelivery_timer"]->get_integer(p_rem));
  ...
  builder::dyn_var<unsigned int> seq =
      net_packet["sequence_number"]->get_integer(p);
  ...
  net_packet["total_len"]->set_integer(p_ack, net_packet.get_total_size() -
                                                  get_headroom());
  net_packet["computed_total_len"]->set_integer(
      p_ack, net_packet.get_total_size() - get_headroom());
  for (builder::static_var<unsigned int> i = m_sequence + 1;
       i < framework::instance.m_registered_modules.size(); i++) {
    builder::static_var<int> s =
        (int)framework::instance.m_registered_modules[i]->hook_send(
            c, p_ack, buff, 1, ret_len_ptr);
...
module::hook_status
reliable_module::hook_establish_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                    builder::dyn_var<connection_t *> c,
                                    builder::dyn_var<unsigned int> remote_host,
                                    builder::dyn_var<unsigned int> remote_app,
                                    builder::dyn_var<unsigned int> local_app) {
...
module::hook_status
reliable_module::hook_send_ctx(builder::dyn_var<runtime::context_t *> ctx,
  ...
  conn_layout.get(c, "redelivery_buffer")[index] = p;
  builder::dyn_var<runtime::timer_t *> t = runtime::alloc_timer_ctx(ctx);
  runtime::insert_timer_ctx(ctx, t,
                            runtime::get_time_ms_now() + REDELIVERY_TIMEOUT_MS,
                            redelivery_timer_callback, p);
...
module::hook_status
reliable_module::hook_ingress_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                  packet_t p) {
  builder::dyn_var<unsigned int> ack_seq =
      net_packet["ack_sequence_number"]->get_integer(p);
  builder::dyn_var<connection_t *> c =
  ...
  if (ack_seq != 0) {
    ...
    if (p_rem != 0) {
      builder::dyn_var<runtime::timer_t *> t = runtime::to_void_ptr(
          net_packet["redelivery_timer"]->get_integer(p_rem));
      runtime::remove_timer_ctx(ctx, t);
      runtime::return_timer_ctx(ctx, t);
      conn_layout.get(c, "redelivery_buffer")[index] = 0;
    }
    return module::hook_status::HOOK_DROP;
  }
  ...
  for (builder::static_var<unsigned int> i = m_sequence + 1;
       i < framework::instance.m_registered_modules.size(); i++) {
    builder::static_var<int> s =
        (int)framework::instance.m_registered_modules[i]->hook_send_ctx(
            ctx, c, p_ack, buff, 1, ret_len_ptr);
    if (s == (int)module::hook_status::HOOK_DROP)
      break;
  }
  return module::hook_status::HOOK_CONTINUE;
