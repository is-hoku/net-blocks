...
void signaling_module::init_module(void) {
  ...
  net_packet.add_member(
      "is_signaling",
      new generic_integer_member<int>((int)member_flags::aligned), 0);
...
module::hook_status signaling_module_after::hook_establish(
  ...
  if (!framework::instance.isIPCompat())
    net_packet["total_len"]->set_integer(p_sig, net_packet.get_total_size() -
                                                    get_headroom() - 1);
  net_packet["computed_total_len"]->set_integer(
      p_sig, net_packet.get_total_size() - get_headroom() - 1);
  for (builder::static_var<unsigned int> i =
           signaling_module::instance.m_sequence + 1;
...
module::hook_status signaling_module_after::hook_establish_ctx(
    builder::dyn_var<runtime::context_t *> ctx,
    builder::dyn_var<connection_t *> c, builder::dyn_var<unsigned int> h,
    builder::dyn_var<unsigned int> a, builder::dyn_var<unsigned int> sa) {

  if (h == get_ctx(ctx, "nb__wildcard_host_identifier") || a == 0) {
    conn_layout.get(c, "signaling_state") = signaling_module::SIGNAL_NA;
    return module::hook_status::HOOK_CONTINUE;
  }
  ...
  for (builder::static_var<unsigned int> i =
    ...
    builder::static_var<int> s =
        (int)framework::instance.m_registered_modules[i]->hook_send_ctx(
            ctx, c, p_sig, buff, 0, ret_len_ptr);
    if (s == (int)module::hook_status::HOOK_DROP)
      break;
  }
...
module::hook_status
signaling_module::hook_ingress_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                   packet_t p) {
  builder::dyn_var<connection_t *> c =
      runtime::to_void_ptr(net_packet["flow_identifier"]->get_integer(p));
  if (conn_layout.get(c, "signaling_state") == WAITING_FOR_SIGNAL)
