...
void routing_module::init_module(void) {
  ...
  if (framework::instance.isIPCompat()) {
    ...
    net_packet.add_member(
        "ip_ttl",
        new generic_integer_member<unsigned char>((int)member_flags::aligned),
        2);
    net_packet.add_member(
        "ip_protocol",
        new generic_integer_member<unsigned char>((int)member_flags::aligned),
...
module::hook_status routing_module::hook_establish(
...
module::hook_status
routing_module::hook_send(builder::dyn_var<connection_t *> c, packet_t p,
                          builder::dyn_var<char *> buff,
                          builder::dyn_var<unsigned int> len,
                          builder::dyn_var<int *> ret_len) {

  // Assuming routing is enabled
  net_packet["dst_global_id"]->set_integer(p,
                                           conn_layout.get(c, "dst_global_id"));
  ...
  if (framework::instance.isIPCompat()) {
    ...
    builder::dyn_var<unsigned char *> header_addr =
        net_packet["ip_VIDE"]->get_addr(p);
    int header_len = 20;

    net_packet["ip_header_checksum"]->set_integer(
        p, runtime::do_ip_checksum(header_addr, header_len));
  }
...
module::hook_status routing_module::hook_ingress(packet_t p) {
  if (framework::instance.isIPCompat()) {
    ...
    builder::dyn_var<unsigned short> total_len =
        runtime::u_ntohs(net_packet["ip_total_len"]->get_integer(p)) + 14;
...
module::hook_status routing_module::hook_establish_ctx(
...
module::hook_status
routing_module::hook_send_ctx(builder::dyn_var<runtime::context_t *> ctx,
  ...
  net_packet["dst_global_id"]->set_integer(p,
                                           conn_layout.get(c, "dst_global_id"));
  net_packet["src_global_id"]->set_integer(p, get_ctx(ctx, "nb__my_host_id"));
...
module::hook_status
routing_module::hook_ingress_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                 packet_t p) {
  if (framework::instance.isIPCompat()) {
    builder::dyn_var<unsigned short> total_len =
        runtime::u_ntohs(net_packet["ip_total_len"]->get_integer(p)) + 14;
