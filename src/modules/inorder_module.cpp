...
void inorder_module::init_module(void) {
  ...
  if (inorder_strategy != no_inorder || reliable_module::instance.is_enabled)
    net_packet.add_member(
        "sequence_number",
        new generic_integer_member<unsigned int>((int)member_flags::aligned),
        4);

  if (inorder_strategy == hold_forever) {
    conn_layout.register_member<builder::dyn_var<void *[INORDER_BUFFER_SIZE]>>(
        "inorder_buffer");
    conn_layout.register_member<builder::dyn_var<unsigned int>>(
        "max_ooo_sequence");
  }
...
module::hook_status
inorder_module::hook_establish(builder::dyn_var<connection_t *> c,
...
module::hook_status
inorder_module::hook_send(builder::dyn_var<connection_t *> c, packet_t p,
                          builder::dyn_var<char *> buff,
                          builder::dyn_var<unsigned int> len,
                          builder::dyn_var<int *> ret_len) {

  builder::dyn_var<unsigned int> sq =
      conn_layout.get(c, "last_sent_sequence") + 1;
...
module::hook_status inorder_module::hook_ingress(packet_t p) {
  ...
  builder::dyn_var<unsigned int> last_seq =
      conn_layout.get(c, "last_recv_sequence");

  if (inorder_strategy == drop_out_of_order) {
    builder::dyn_var<unsigned int> packet_seq =
    ...
    if (last_seq < packet_seq) {
      ...
      builder::dyn_var<int> payload_len =
          net_packet["computed_total_len"]->get_integer(p) -
          ((net_packet.get_total_size()) - 1 - get_headroom());
      if (payload_len != 0)
        runtime::insert_data_queue(conn_layout.get(c, "input_queue"),
    ...
    if (last_seq == 0 || packet_seq == last_seq + 1) {
      ...
      if (payload_len != 0)
        runtime::insert_data_queue(conn_layout.get(c, "input_queue"),
                                   ...
                                   payload_len);
      conn_layout.get(c, "last_recv_sequence") = packet_seq;
      // Now that we have received this packet, maybe we have more out of order
      // packets to deliver
      for (builder::dyn_var<unsigned int> i = packet_seq + 1;
           i <= conn_layout.get(c, "max_ooo_sequence"); i = i + 1) {
...
module::hook_status
inorder_module::hook_establish_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                   builder::dyn_var<connection_t *> c,
                                   builder::dyn_var<unsigned int> remote_host,
                                   builder::dyn_var<unsigned int> remote_app,
                                   builder::dyn_var<unsigned int> local_app) {
...
module::hook_status
inorder_module::hook_send_ctx(builder::dyn_var<runtime::context_t *> ctx,
...
module::hook_status
inorder_module::hook_ingress_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                 packet_t p) {
  // At this point the identifier module has run, so it is safe to look up the
  // flow identifier from the non-send headers
  builder::dyn_var<connection_t *> c =
