...
static void debug_send_drop(module *m, packet_t p, builder::dyn_var<int> len) {
  if (debug_module_paths) {
    runtime::info_log("Packet [%p, %d] dropped on send path",
                      m->get_module_name(), p, len);
  }
}
static void debug_ingress_drop(module *m, packet_t p,
                               builder::dyn_var<int> len) {
  if (debug_module_paths) {
    runtime::info_log("Packet [%p, %d] dropped on ingress path",
                      m->get_module_name(), p, len);
  }
}
...
void framework::run_establish_path(builder::dyn_var<connection_t *> conn,
                                   builder::dyn_var<unsigned int> host_id,
                                   builder::dyn_var<unsigned int> app_id,
                                   builder::dyn_var<unsigned int> src_app_id) {

  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
    builder::static_var<int> s = (int)m_registered_modules[i]->hook_establish(
        conn, host_id, app_id, src_app_id);
...
void framework::run_destablish_path(builder::dyn_var<connection_t *> c) {
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
    ...
    builder::static_var<int> s =
        (int)m_registered_modules[i]->hook_destablish(c);
    if (s == (int)module::hook_status::HOOK_DROP) {
...
builder::dyn_var<int>
framework::run_send_path(builder::dyn_var<connection_t *> conn,
  ...
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
    builder::static_var<int> s = (int)m_registered_modules[i]->hook_send(
        conn, p, buff, len, ret_len_ptr);
    if (s == (int)module::hook_status::HOOK_DROP) {
...
void framework::run_ingress_path(packet_t p) {
  for (builder::static_var<int> i = m_registered_modules.size() - 1; i >= 0;
       i--) {
...
void framework::run_net_init_path(void) {
  runtime::net_state_obj =
      runtime::malloc(runtime::size_of(runtime::net_state_obj[0]));
  runtime::nb_assert(runtime::net_state_obj != 0,
                     "malloc failed while allocating net_state");
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
...
// ==================== Context-based path implementations ====================

void framework::run_establish_path_ctx(
  ...
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
    builder::static_var<int> s =
        (int)m_registered_modules[i]->hook_establish_ctx(ctx, conn, host_id,
                                                         app_id, src_app_id);
    if (s == (int)module::hook_status::HOOK_DROP) {
      debug_establish_drop(m_registered_modules[i]);
      break;
...
void framework::run_destablish_path_ctx(
    builder::dyn_var<runtime::context_t *> ctx,
    builder::dyn_var<connection_t *> c) {
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
    builder::static_var<int> s =
        (int)m_registered_modules[i]->hook_destablish_ctx(ctx, c);
    if (s == (int)module::hook_status::HOOK_DROP) {
      debug_destablish_drop(m_registered_modules[i]);
      break;
    }
...
builder::dyn_var<int>
framework::run_send_path_ctx(builder::dyn_var<runtime::context_t *> ctx,
                             builder::dyn_var<connection_t *> conn,
                             builder::dyn_var<char *> buff,
                             builder::dyn_var<int> len) {
  ...
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
    builder::static_var<int> s = (int)m_registered_modules[i]->hook_send_ctx(
        ctx, conn, p, buff, len, ret_len_ptr);
    if (s == (int)module::hook_status::HOOK_DROP) {
      debug_send_drop(m_registered_modules[i], p, len);
      break;
    }
...
void framework::run_ingress_path_ctx(builder::dyn_var<runtime::context_t *> ctx,
                                     packet_t p) {
  for (builder::static_var<int> i = m_registered_modules.size() - 1; i >= 0;
       i--) {
    builder::static_var<int> s =
        (int)m_registered_modules[i]->hook_ingress_ctx(ctx, p);
    if (s == (int)module::hook_status::HOOK_DROP) {
      debug_ingress_drop(m_registered_modules[i], p, 0);
      break;
    }
...
void framework::run_net_init_path_ctx(
    builder::dyn_var<runtime::context_t *> ctx) {
  // For context-based version, net_state is already allocated in the context
  // Just run the init hooks
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
    m_registered_modules[i]->hook_net_init_ctx(ctx);
  }
}
...
int get_headroom(void) { return net_packet.group_start_offset(1); }
