...
dynamic_object ctx_layout;

int dynamic_object::dynamic_object_counter = 0;
void dynamic_object::generate_struct_decl(std::ostream &oss,
                                          std::string struct_name) {
  oss << "struct dynamic_struct_" << dynamic_object_counter << ";" << std::endl;
  oss << "typedef struct dynamic_struct_" << dynamic_object_counter << " "
      << struct_name << ";" << std::endl;
  oss << "struct dynamic_struct_" << dynamic_object_counter << " {"
      << std::endl;
...
builder::builder dynamic_object::get(builder::dyn_var<connection_t *> c,
                                     std::string name) {
  ...
  // to the map
  return (builder::builder)(builder::dyn_var<int>)builder::as_member(
      ((builder::dyn_var<connection_t>)(builder::cast)(c[0])).addr(), name);
}
