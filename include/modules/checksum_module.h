...
public:
  enum checksum_type_t { full_packet, header_only };
...
public:
  ...
  void configEnableChecksum(void) { is_enabled = true; }
  void configDisableChecksum(void) { is_enabled = false; }

  void configChecksumType(checksum_type_t ct) { checksum_type = ct; }
