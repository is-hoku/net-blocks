...
struct dynamic_object {
  ...
  template <typename T> void register_member(std::string s) {
...
// Helper function to access runtime state from context
static inline builder::builder
get_state_ctx(builder::dyn_var<runtime::context_t *> ctx, std::string name) {
  builder::dyn_var<void *> ctx_net_state = ctx_layout.get(ctx, "nb__net_state");
  return net_state.get(ctx_net_state, name);
}

// Helper to get context member
static inline builder::builder
get_ctx(builder::dyn_var<runtime::context_t *> ctx, std::string name) {
  return ctx_layout.get(ctx, name);
}
