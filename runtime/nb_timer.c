...
nb__timer *nb__alloc_timer_ctx(nb__context_t *ctx) {
  if (ctx->nb__timer_free_list == NULL)
    return NULL;
  nb__timer *to_ret = ctx->nb__timer_free_list;
  ctx->nb__timer_free_list = to_ret->next;
  return to_ret;
}
...
void nb__return_timer_ctx(nb__context_t *ctx, nb__timer *t) {
  t->next = ctx->nb__timer_free_list;
  ctx->nb__timer_free_list = t;
}
...
void nb__insert_timer_ctx(nb__context_t *ctx, nb__timer *t,
  ...
  // We find curr such that curr is the timer
  // _after_ where to insert
  nb__timer *curr = ctx->nb__timers_head;
  nb__timer *prev = NULL;
  while (curr != NULL) {
    if (curr->timeout > to)
      break;
...
void nb__remove_timer_ctx(nb__context_t *ctx, nb__timer *t) {
  if (t->next) {
    t->next->prev = t->prev;
  }
  if (t->prev) {
...
void nb__init_timers_ctx(nb__context_t *ctx) {
  ctx->nb__timer_free_list = &(ctx->nb__allocated_timers[0]);
  for (int i = 0; i < MAX_TIMER_ALLOCS - 1; i++) {
    ctx->nb__allocated_timers[i].next = &(ctx->nb__allocated_timers[i + 1]);
  }
  ctx->nb__allocated_timers[MAX_TIMER_ALLOCS - 1].next = NULL;
  ctx->nb__last_timer_checked = nb__get_time_ms_now();
}
...
void nb__check_timers_ctx(nb__context_t *ctx) {
  unsigned long long t_now = nb__get_time_ms_now();
  // Fire all timers who's timeout <= time now

  while (ctx->nb__timers_head != NULL) {
    if (ctx->nb__timers_head->timeout <= t_now) {
      nb__timer *t = ctx->nb__timers_head;
      nb__remove_timer_ctx(ctx, t);
      // We are firing the callback with current time as the third argument
      t->callback(t, t->argument, t_now);
    } else
      break;
  ...
  ctx->nb__last_timer_checked = t_now;
}
