...
void nb__free_data_queue(struct data_queue_t *q) { free(q); }
void nb__insert_data_queue(struct data_queue_t *q, char *buff, int len) {
  if (q == NULL) {
    fprintf(stderr, "ERROR: nb__insert_data_queue: queue is NULL\n");
    return;
  }

  int index = q->current_elems;
  if (index >= 512) { // MAX_DATA_QUEUE_ELEMS
    fprintf(stderr,
            "ERROR: nb__insert_data_queue: queue overflow, current_elems=%d\n",
            index);
    return;
  }
...
void nb__free_accept_queue(nb__accept_queue_t *q) { free(q); }
void nb__insert_accept_queue(nb__accept_queue_t *q, unsigned src_app_id,
                             unsigned long long src_host_id, void *packet) {
  if (q == NULL) {
    fprintf(stderr, "ERROR: nb__insert_accept_queue: queue is NULL\n");
    return;
  }

  int index = q->current_elems;
  if (index >= 512) { // MAX_ACCEPT_QUEUE_ELEMS
    fprintf(
        stderr,
        "ERROR: nb__insert_accept_queue: queue overflow, current_elems=%d\n",
        index);
    return;
  }
...
int nb__read(nb__connection_t *c, char *buff, int max_len) {
  if (c == NULL || c->input_queue == NULL) {
    fprintf(stderr, "ERROR: nb__read: connection or input_queue is NULL\n");
    return -1;
  }
  if (buff == NULL || max_len <= 0) {
    fprintf(stderr, "ERROR: nb__read: invalid buffer parameters\n");
    return -1;
  }
  ...
  if (payload == NULL || payload_size < 0) {
    fprintf(stderr, "ERROR: nb__read: invalid payload data\n");
    return -1;
  }
...
nb__connection_t *nb__accept(nb__connection_t *c,
                             void (*callback)(int, nb__connection_t *)) {
  ...
  nb__connection_t *s =
      nb__establish(src_host_id, src_app_id, dst_app_id, callback);
...
nb__connection_t *nb__accept_ctx(nb__context_t *ctx, nb__connection_t *c,
                                 void (*callback)(int, nb__connection_t *)) {
  if (c->accept_queue->current_elems == 0)
    return NULL;
  ...
  void *packet = c->accept_queue->packet[index];
  nb__connection_t *s =
      nb__establish_ctx(ctx, src_host_id, src_app_id, dst_app_id, callback);
  // For an accepted connection, we have already been signalled
  s->signaling_state = SIGNALED;
  if (packet)
    nb__run_ingress_step_ctx(ctx, packet, 0);
...
static void nb__cycle_connections_ctx(nb__context_t *ctx) {
  int i;
  unsigned long long time_now = nb__get_time_ms_now();
  for (i = 0; i < ctx->nb__net_state->num_conn; i++) {
    nb__connection_t *c = ctx->nb__net_state->active_connections[i];
...
void nb__main_loop_step(void) {
  // Add safety check for global state
  if (nb__net_state == NULL) {
    fprintf(stderr, "ERROR: nb__main_loop_step: nb__net_state is NULL\n");
    return;
  }
...
void nb__main_loop_step_ctx(nb__context_t *ctx) {
  ...
  struct timespec tv;
  clock_gettime(CLOCK_MONOTONIC, &tv);
  ctx->nb__time_now = tv.tv_sec * 1000 + tv.tv_nsec / 1000000;
  ...
  // Currently just processing one packet
  p = nb__poll_packet_ctx(ctx, &len, nb__packet_headroom);
  if (p != NULL) {
    nb__run_ingress_step_ctx(ctx, p, len);
  }

  nb__cycle_connections_ctx(ctx);

  nb__check_timers_ctx(ctx);
}
...
nb__routing_table_lookup_from_global(unsigned int local_id,
  ...
  // When routing table is not initalized, the local ids are the same as the
  // global ids
  if (entries == NULL)
